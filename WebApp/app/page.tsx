...
const STORAGE_KEY = "sakhya-web-v1";
...
export default function SakhyaWebApp() {
  ...
  useEffect(() => {
    ...
    if (saved) {
      try {
        // Hydrate the browser's private offline workspace after the server render.
        // eslint-disable-next-line react-hooks/set-state-in-effect
        setState(JSON.parse(saved) as PersistedState);
      } catch {
        setNotice("Your local data could not be read, so the sample workspace is shown.");
      }
    }
    setHydrated(true);
  }, []);
  ...
  function attachPhoto(event: ChangeEvent<HTMLInputElement>) {
    const file = event.target.files?.[0];
    if (!file) return;
    if (file.size > 1_500_000) {
      setNotice("Choose an image under 1.5 MB so it can remain safely in local browser storage.");
      return;
    }
    const reader = new FileReader();
    reader.onload = () => setCapturePhoto(String(reader.result));
  ...
  function importData(event: ChangeEvent<HTMLInputElement>) {
    ...
    reader.onload = () => {
      try {
        setState(JSON.parse(String(reader.result)) as PersistedState);
  ...
  function resetData() {
    if (!window.confirm("Replace this browser’s data with the sample workspace? Export first if you want a backup.")) return;
    setState(seedState);
    setNotice("Sample workspace restored in this browser.");
  }
          ...
          <div className="capture-footer">
            <div className="capture-tools">
              <label className="tool-button">
                ...
                <input type="file" accept="image/*" onChange={attachPhoto} hidden />
              </label>
    ...
    <div className="app-frame">
      <aside className={`sidebar ${mobileMenu ? "open" : ""}`}>
        ...
        <div className="sidebar-bottom">
          <div className="sync-state"><span /><div><strong>Saved locally</strong><small>Private to this browser</small></div></div>
      ...
      {settingsOpen && (
        <div className="modal-layer" role="dialog" aria-modal="true" aria-label="Data settings">
          ...
          <section className="settings-modal">
            ...
            <div className="data-summary"><ShieldCheck size={23} /><div><strong>Local and private</strong><span>Your web data stays in this browser until you export it.</span></div></div>
